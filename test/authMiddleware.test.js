...
describe("verifyToken middleware", () => {
    ...
    it("should return 401 if an operational user try to create a product", async() => {
        req.method = "POST"
        const res = mockResponse()
    ...
    it("should return 401 if an operational user try to update a product", async() => {
        req.method = "PUT"
        const res = mockResponse()
        const token = jwt.sign({ id:'000898'}, process.env.JWT_SECRET);
        req.headers = { authorization: `Bearer ${token}` }
    ...
    it("should return 401 if an operational user try to delete", async() => {
        req.method = "DELETE"
        const res = mockResponse()
        const token = jwt.sign({ id:'000898'}, process.env.JWT_SECRET);
        req.headers = { authorization: `Bearer ${token}` }
